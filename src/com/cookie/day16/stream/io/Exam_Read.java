...
public class Exam_Read {
	public static void main(String[] args) { // 메인을 기준으로 입출력 판단필요
		...
		try {
			...
			Reader fr = new FileReader("src/com/cookie/day16/stream/exercise/Exercise_MemoRead.java");
			
			int readCount;
			byte [] readBytes = new byte[1024];
			readCount = is.read(readBytes);
			String result = new String(readBytes, 0, readCount); // 배열이 ㅣ크면 한전에 읽어 올 수 있어서 반복하지 않아도 되지만 
